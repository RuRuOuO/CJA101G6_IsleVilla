...
public interface RoomRepository extends JpaRepository<Room, Integer>,RoomInterface{
	//計算每個房型數量
	 @Query("SELECT new com.islevilla.chen.room.model.RoomTypeCount(r.roomTypeId, CAST(COUNT(r) AS integer), 0, 0) " +
	           "FROM Room r " +
	 ...
	 @Query("SELECT new com.islevilla.chen.room.model.RoomTypeCount(r.roomTypeId,  0, CAST(COUNT(r) AS integer),0) " +
		       "FROM Room r " +
